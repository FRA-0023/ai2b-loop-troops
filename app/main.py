...
if not st.session_state.get("authenticated", False):
    ...
    with auth_col2:
        ...
        if st.button("🔐 Sign In to FinSight Workspace", type="primary", use_container_width=True):
...
if st.sidebar.button("⚡ EcoTex Milano (€750k CapEx)", use_container_width=True):
...
if st.session_state.get("user_role") == "accounting_advisor":
    if st.sidebar.button("⚡ Meccanica Varese (€500k CNC)", use_container_width=True):
    ...
    if st.sidebar.button("⚡ AgroBio Brianza (€400k Bio-Pack)", use_container_width=True):
...
with tab_ingest:
    ...
    with up_col1:
        ...
        if uploaded_file is not None:
            if st.button("⚡ Parse & Ingest into DuckDB", type="primary", use_container_width=True):
...
with tab_export:
    ...
    with col_exp2:
        ...
        if st.button("🖨️ Generate Bank PDF Passport", use_container_width=True):
